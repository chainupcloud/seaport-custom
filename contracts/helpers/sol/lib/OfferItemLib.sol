...
import {ConsiderationItem, OfferItem, SpentItem} from "../../../lib/ConsiderationStructs.sol";
...
library OfferItemLib {
    bytes32 private constant OFFER_ITEM_MAP_POSITION = keccak256("seaport.OfferItemDefaults");
    bytes32 private constant OFFER_ITEMS_MAP_POSITION = keccak256("seaport.OfferItemsDefaults");
    bytes32 private constant EMPTY_OFFER_ITEM = keccak256(
        abi.encode(
            OfferItem({itemType: ItemType(0), token: address(0), identifierOrCriteria: 0, startAmount: 0, endAmount: 0})
        )
    );
     ...
     */
    function fromDefault(string memory defaultName) internal view returns (OfferItem memory item) {
        mapping(string => OfferItem) storage offerItemMap = _offerItemMap();
     ...
     */
    function fromDefaultMany(string memory defaultsName) internal view returns (OfferItem[] memory items) {
        mapping(string => OfferItem[]) storage offerItemsMap = _offerItemsMap();
     ...
     */
    function saveDefaultMany(OfferItem[] memory offerItems, string memory defaultsName)
        internal
        returns (OfferItem[] memory _offerItems)
    {
        mapping(string => OfferItem[]) storage offerItemsMap = _offerItemsMap();
        OfferItem[] storage items = offerItemsMap[defaultsName];
     ...
     */
    function copy(OfferItem memory item) internal pure returns (OfferItem memory) {
        return OfferItem({
     ...
     */
    function _offerItemMap() private pure returns (mapping(string => OfferItem) storage offerItemMap) {
     ...
     */
    function _offerItemsMap() private pure returns (mapping(string => OfferItem[]) storage offerItemMap) {
     ...
     */
    function withItemType(OfferItem memory item, ItemType itemType) internal pure returns (OfferItem memory) {
     ...
     * @custom:return _offerItem the modified OfferItem
     */
    function withToken(OfferItem memory item, address token) internal pure returns (OfferItem memory) {
        item.token = token;
        return item;
    }
     ...
     * @custom:return _offerItem the modified OfferItem
     */
    function withIdentifierOrCriteria(OfferItem memory item, uint256 identifierOrCriteria)
        internal
        pure
     ...
     * @custom:return _offerItem the modified OfferItem
     */
    function withStartAmount(OfferItem memory item, uint256 startAmount) internal pure returns (OfferItem memory) {
     ...
     * @custom:return _offerItem the modified OfferItem
     */
    function withAmount(OfferItem memory item, uint256 amount) internal pure returns (OfferItem memory) {
     ...
     */
    function toSpentItemArray(OfferItem[] memory items) internal pure returns (SpentItem[] memory) {
